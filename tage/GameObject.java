...
public class GameObject
{
	...
	*/
	...
	/*  rotates the object on its up axis */

	public void yaw(float movmentSpeed){
		Vector4f oldUp;
		Matrix4f oldRotation, rotAroundGoUp;
		oldRotation = new Matrix4f(this.getWorldRotation());
		oldUp = new Vector4f(0f,1f,0f,1f).mul(oldRotation);
		rotAroundGoUp = new Matrix4f().rotation(movmentSpeed,
			new Vector3f(oldUp.x(), oldUp.y(), oldUp.z()));
		oldRotation.mul(rotAroundGoUp);
		setLocalRotation(oldRotation);
	}

	/* moves object forwards or backwards  */

	public void moveToFro(float movement){
		
		Vector3f newLocation;
		newLocation = this.getWorldLocation().add(this.getWorldForwardVector().mul(movement));
		newLocation = new Vector3f(newLocation.x(),0,newLocation.z());
		this.setLocalLocation(newLocation);
	}

	/* rotates object around right facing axis  */
	public void pitch(float movement){
		Vector4f oldUp;
		Matrix4f oldRotation, rotAroundGoRight, newRotation;
		oldRotation = new Matrix4f(this.getWorldRotation());
		oldUp = new Vector4f(1f,0f,0f,1f).mul(oldRotation);
		rotAroundGoRight = new Matrix4f().rotation(movement,
			new Vector3f(oldUp.x(), oldUp.y(), oldUp.z()));
		rotAroundGoRight.mul(oldRotation);
		this.setLocalRotation(rotAroundGoRight);
	}
}